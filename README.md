...
For verified source, Mezo testnet routes chain id `31611` through Blockscout's v2 smart-contract endpoint at `https://api.explorer.test.mezo.org/api/v2` instead of Sourcify or Etherscan, which do not cover Mezo today. The engine maps Blockscout's `source_code` plus `additional_sources[]` payload into the same compiled artifact shape used by the existing source pipeline, and because Mezo has no Etherscan creation-transaction API, the debug path falls back to installing the recompiled deployed runtime directly for existing verified contracts. Together those paths unlock recompiled source maps, source-backed rendered trace rows, and debugger hook snapshots for verified Mezo contracts. Mezo mainnet Blockscout support is left as a TODO until its API base URL is published.
